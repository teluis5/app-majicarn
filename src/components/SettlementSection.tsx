    ...
    <div className="pt-3 border-t border-slate-200/80 space-y-3">
      ...
      <div>
        <label className="block text-[11px] font-bold text-slate-500 mb-1 flex items-center gap-1">
          ...
          送金先メモ (PayPayリンクや口座番号等 / 任意)
        </label>
        <input
          type="text"
          value={payPayId}
          onChange={(e) => onPayPayIdChange(e.target.value)}
          placeholder="paypay.me/xxxx など（LINEテキストに自動追記されます）"
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="py-3 px-3 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-extrabold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-1.5 shadow-xs transition-all"
        >
          {copied ? (
            <>
        ...
        <button
          ...
          className="py-3 px-3 bg-slate-100 hover:bg-slate-200 active:scale-[0.98] text-slate-700 font-bold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-1.5 transition-all"
        >
